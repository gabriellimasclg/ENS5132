...
def airQualityHist(aqData,stations,uf,repoPath):
    os.makedirs(repoPath+'/figures/'+uf, exist_ok=True)
    
    for st in stations:
        fig, ax = plt.subplots()
        aqData[aqData.Estacao==st].hist('Valor',by='Poluente', ax=ax)
        fig.savefig(repoPath+'/figures/'+uf+'/hist_'+st+'.png')
    ...
    for st in stations:
        ...
        for ii, pol in enumerate(pollutants):
            if pollutants.size>1:
                ...
                fig.savefig(repoPath+'/figures/'+uf+'/plot_'+st+'.png')
            else:
                ax.plot(
                    aqData[(aqData.Estacao==st) & (aqData.Poluente == pol)].Valor)
                fig.savefig(repoPath+'/figures/'+uf+'/plot_'+st+'.png')
...
def normalityCheck(aqTableAlvo, repoPath, uf, stationAlvo, pol):
    ...
    data = aqTableAlvo[pol].dropna()
    data_positive = data[data > 0]

    ax[0].hist(np.log(data_positive), facecolor='red')
    ax[0].set_title('Log')
    ax[0].set_ylabel('Frequência')

    ax[1].hist(stats.boxcox(data_positive)[0], facecolor='green')
    ...
    fig.tight_layout()
    fig.savefig(repoPath + '/figures/' + uf + '/histogramDataNormalization_' + pol + '_' + stationAlvo + '.png')
    return fig
...
def timeSeriesForecast(complete_data,repoPath,uf,pol,stationAlvo):
    ...
    fig.savefig(repoPath+'/figures/'+uf+'/timeSeriesForecast'+pol+'_'+stationAlvo+'.png')
    return fig