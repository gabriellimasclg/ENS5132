...
def multivariateStatistics(aqTable,stations):
    
    aqTableAlvo = aqTable.reset_index()[aqTable.reset_index()['Estacao'] == 'Centro - Av. do Contorno'].iloc[:, 3:]
    ...
    #regplot
    fig, ax = plt.subplots()
    sns.regplot(x= 'O3', y= 'NO2', data = aqTableAlvo)
    ...
    regData = np.array(aqTableAlvo[['MP10','NO2','O3']].copy().dropna(axis=0))
